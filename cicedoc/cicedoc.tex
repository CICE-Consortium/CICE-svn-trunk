...
Software User's Manual \\ Version 5.1  \\ LA-CC-06-012 }
...
This model release is CICE version 5.1, available from  
%\href{http://climate.lanl.gov/Models/CICE/}{http://climate.lanl.gov/Models/CICE/}.  
\href{http://oceans11.lanl.gov/trac/CICE/wiki}{http://oceans11.lanl.gov/trac/CICE/wiki}. 
It updates CICE 5.0, which was released in September 2013: 
\begin{itemize}
\item include ice velocity in atm-ice coupling updates (e.g. stress) for high-frequency coupling
\item allow a variable coefficient for the ice-ocean heat flux
\item several new namelist options improve flexibility, especially for coupled model configurations:
\begin{itemize}
\item	ice-ocean heat flux
\item	 `virtual' or `real' topo melt pond water
\item	ocean freezing temperature
\item	high-frequency coupling
\item coupling and computational grids may be different
\item	and more
\end{itemize}
\item additional software enhancements improve flexibility and compatibility with CESM, Hadley Centre, and U.S. Navy coupled models
\item new diagnostics and updated documentation
\item various bug fixes 
\end{itemize}
The previous version, CICE v5.0, includes the following improvements:
\begin{itemize}
\item A method for prognosing sea ice salinity, including improved snow-ice formation
...
\item The ``revised EVP'' under-damping approach
...
\item Ice and snow enthalpy defined as tracers
...
\item Restarts in binary or netCDF formats
...
\href{http://oceans11.lanl.gov/trac/CICE/wiki}{http://oceans11.lanl.gov/trac/CICE/wiki} \\
\href{http://oceans11.lanl.gov/trac/CICE/wiki/CiceUsers}{http://oceans11.lanl.gov/trac/CICE/wiki/CiceUsers} \\
\href{http://oceans11.lanl.gov/trac/CICE/wiki/CiceCodeModifications}{http://oceans11.lanl.gov/trac/CICE/wiki/CiceCodeModifications} \\
\href{http://oceans11.lanl.gov/trac/CICE/wiki/CiceDev}{http://oceans11.lanl.gov/trac/CICE/wiki/CiceDev}
%\href{http://oceans11.lanl.gov/drupal/CICE/}{http://oceans11.lanl.gov/drupal/CICE} \\
%\href{http://oceans11.lanl.gov/drupal/CICE/Users}{http://oceans11.lanl.gov/drupal/CICE/Users} \\
%\href{http://oceans11.lanl.gov/drupal/CICE/Code-Modifications}{http://oceans11.lanl.gov/drupal/CICE/Code-Modifications} \\
%\href{http://oceans11.lanl.gov/drupal/CICE/Collaborative-Projects}{http://oceans11.lanl.gov/drupal/CICE/Collaborative-Projects}
...
By convention, directional fluxes are positive downward.  In CESM, the sea ice model may exchange coupling fluxes using a different grid than the computational grid.  This functionality is activated using the namelist variable {\tt gridcpl\_file}.  Another namelist variable {/tt highfreq}, allows the high-frequency coupling\index{coupling!high-frequency} procedure implemented in the Regional Arctic System Model (RASM).\index{RASM}  In particular, the relative atmosphere-ice velocity ($\vec{U}_a-\vec{u}$) is used instead of the full atmospheric velocity for computing turbulent fluxes in the atmospheric boundary layer.
...
turbulent scales. The number of iterations is set by the namelist variable {\tt natmiter}.  (For the case with form drag, see section~\ref{sec:formdrag}.)  Here, $z_{\mathit {ref}}$ is a 
\index{height!reference}%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\markboth{Coupling with other climate model components}{Variable exchange coefficients}
...
\index{ridging}%
\index{melt pond}%
The contributions to form drag from ridges (and keels\index{keels} underneath the ice), floe edges  and  melt pond edges can be expressed using the general formulation of equation \eqref{FormulaDragCoeff} (see \cite{TFSFFKLB:14} for details). Individual terms in equation \eqref{Cda} are fully described in \cite{TFSFFKLB:14}. Following \cite{Arya:75} the skin drag coefficient is parametrized as 
\begin{equation}
...
\index{floe size}
$L_A$, distance between floes, $D_F$, and melt pond size, $L_P$ we use the parameterizations of \cite{LGHA:12} to relate these quantities to the ice and pond concentrations. All of these intermediate quantities are available as history\index{history} output, along with {\tt Cdn\_atm}, {\tt Cdn\_ocn} and the ratio {\tt Cdn\_atm\_ratio\_n} between the total atmospheric drag and the atmospheric neutral drag coefficient.
...
\index{stability}%
and unstable atmosphere--ice boundary layers. In contrast to the neutral drag coefficients the stability effect of the atmospheric boundary layer is calculated separately for each ice thickness category. 

The transfer coefficient for oceanic heat flux to the bottom of the ice may be varied based on form drag considerations by setting the namelist variable {\tt fbot\_xfer\_type} to {\tt 'Cdn\_ocn'}; this is recommended when using the form drag parameterization. Its default value of the transfer coefficient is 0.006 ({\tt fbot\_xfer\_type = 'constant'}).
...
$N_C$ & 5 & 5 & 5 & 6 & 7 \\
\hline
categories & \multicolumn{5}{c}{lower bound (m)}  \\
...
\end{tabular}
\end{center}
\caption{\label{table:itd} Lower boundary values for thickness categories, in meters, for the three distribution options ({\tt kcatbound}) and linear remapping ({\tt kitd} = 1).  In the WMO\index{WMO} case, the distribution used depends on the number of categories used.}
\end{table}
...
Setting the flag {\tt skl\_bgc} to true turns on a reduced `NP'
...
formulation ensures small Darcy velocities\index{Darcy flow}
 ...
 Both the volume fraction $f_{bri}$ and the area-weighted brine height $h_b$ can be written to the history file.\index{history}  The history variable {\tt fbri} contains the volume-averaged brine fraction tracer
 \[{{\sum f_{bri} v_i} \over {\sum v_i}},\]
 while {\tt hbri} is comparable to {\tt hi} ($h_i$)
...
When the subcycling\index{subcycling} is finished for each (thermodynamic) time step, the ice--ocean stress\index{ice--ocean stress} must be constructed from {\tt taux(y)} and the terms containing {\tt vrel} on the left  hand side of the equations.  

The Hibler-Bryan form for the ice-ocean stress \cite{HB:87} is included in {\bf ice\_dyn\_shared.F90} but is currently commented out, pending further testing.
...
\end{equation}
This is equivalent to Eq.~(23) in \cite{BFLM:13}, but using $\sigma$ at the current subcycle $k+1$ in the last term on the right-hand side.  Likewise, setting 
\[\alpha_2 = {2T\over e^2\Delta t_e} = {\alpha_1\over e^2}\]
produces equations equivalent to Eq.~(23) in \cite{BFLM:13} for $\sigma_2$ and $\sigma_{12}$.  Therefore the only change needed in the stress code is to use $\alpha_1$ and $\alpha_2$ instead of  $2T / \Delta t_e$ and $2T /e^2 \Delta t_e$.
...
The net surface energy flux from the atmosphere to the ice
...
An option for zero-layer thermodynamics \cite{Semt:76} is available in this version of CICE by setting the namelist \index{namelist}parameter {\tt ktherm} to 0 and changing the number of ice layers, {\tt nilyr}, in {\bf ice\_domain\_size.F90} to 1.  In the zero-layer case, the ice is fresh and the thermodynamic calculations are much simpler than in the other configurations, which we describe here.\\

\markboth{Model components}{Bitz and Lipscomb thermodynamics}
...
\markboth{Model components}{Mushy thermodynamics}
...
{\it Tracer consistency.}\index{tracers}
...
different processors in a distributed system.  The parallelization
...
{\tt NTASK} and {\tt THRD}
definitions in the compile script.  Message passing between the
...
\begin{description}
\item[{\bf cice/}]  
\item[{\bf   README\_v5.1}]     basic information
...
\begin{description}
\item[{\bf cice/}]  official driver for CICE v5 (LANL)
  \begin{description}
  ...
  \item[{\bf   CICE\_RunMod.F90\_debug}]   debugging version of {\bf CICE\_RunMod.F90}
...
\item[{\bf   gx1/}] $\left<1^\circ\right>$ displaced pole grid\index{grid!horizontal} files
  ...
  \item[{\bf   iced\_gx1\_v5.nc}] \netcdf\ restart file used for initial condition\index{initial condition} 
  \end{description}
\item[{\bf   gx3/}] $\left<3^\circ\right>$ displaced pole grid\index{grid!horizontal} files
  ...
  \item[{\bf   ice.restart\_file}] pointer for restart file name\index{restart}
  \item[{\bf   ice\_in}] namelist input data (data paths depend on particular system)\index{namelist}
  \item[{\bf   iced\_gx3\_v5.nc}] \netcdf\ restart file used for initial condition\index{initial condition} 
  \end{description}
\item[convert\_restarts.f90] Fortran code to convert restart files from v4.1 to v5 (4 ice layers)
...
\end{description}
\item[{\bf   io\_binary/}] binary history and restart modules\index{history}
  \begin{description}
  \item[{\bf   ice\_history\_write.F90}] subroutines with binary output 
  \item[{\bf   ice\_restart.F90}]  read/write binary restart files \index{restart}
  \end{description}
\item[{\bf   io\_netcdf/}] \netcdf\ history and restart  modules\index{history}
  \begin{description}
  \item[{\bf   ice\_history\_write.F90}] subroutines with \netcdf\ output
  \item[{\bf   ice\_restart.F90}]  read/write \netcdf\  restart files \index{restart}
  \end{description}
\item[{\bf   io\_pio/}] parallel I/O history and restart  modules\index{history}\index{parallelization}\index{PIO}
  \begin{description}
  \item[{\bf   ice\_history\_write.F90}] subroutines with \netcdf\  output using PIO
  \item[{\bf   ice\_pio.F90}] subroutines specific to PIO
  \item[{\bf   ice\_restart.F90}]  read/write \netcdf\ restart files using PIO \index{restart}
  \end{description}
...
\item[{\bf   ice\_history\_drag.F90}] history output of form drag variables\index{history}
...
\item[{\bf   ice\_restart\_driver.F90}]  driver for reading/writing  restart files\index{restart}
\item[{\bf   ice\_restart\_shared.F90}]  code shared by all  restart options\index{restart}
...
\item[{\bf history/iceh.[timeID].nc}]   output history file \index{history}
\item[{\bf kmt}]  land mask file from {\bf cice/input\_templates/gx3/}\index{masks}
\item[{\bf restart/}]  restart directory \index{restart}
\begin{description}
\item[{\bf iced\_gx3\_v5.nc}]  initial condition from {\bf cice/input\_templates/gx3/}\index{initial condition}
...
problems associated with the B-grid.  EVP is available on the C-grid through the MITgcm code distribution, 
\href{http://mitgcm.org/viewvc/MITgcm/MITgcm/pkg/seaice/}{http://mitgcm.org/viewvc/MITgcm/MITgcm/pkg/seaice/}.
\index{C-grid}%
...
lengths and other parameters for a nonuniform grid (including tripole and regional grids),\index{grid!tripole}\index{grid!regional}
...
In CESM,\index{CESM/CCSM} 
the sea ice model may exchange coupling fluxes using a different grid than the computational grid.  This functionality is activated using the namelist variable {\tt gridcpl\_file}.\index{flux coupler}
\begin{figure}
\centerline{
...
Alternatively, a new variable is provided in the history files, {\tt blkmask}, which labels the blocks in the grid decomposition according to {\tt blkmask = my\_task + iblk/100}.
\index{blocks|)} 
...
Open/cyclic boundary conditions are the default in CICE; the physical domain can 
still be closed using the land mask.
...
The namelist\index{namelist} variable {\tt restore\_ice} turns this functionality on and off; the restoring timescale {\tt trestore} may be used (it is also used for restoring ocean sea surface temperature\index{temperature!ocean} in stand-alone ice runs).  This implementation is only intended to provide the ``hooks" for a more sophisticated 
treatment; the rectangular grid option can be used to test this
configuration.   The `displaced\_pole' grid option should not be used unless the regional grid contains land all along the north and south boundaries.  The current form of the boundary condition routines does not allow Neumann boundary conditions, which must be set explicitly.  This has been done in an unreleased branch of the code; contact Elizabeth for more information.

For exact restarts\index{restart} using restoring, set {\tt restart\_ext} = true in namelist\index{namelist} to use the extended-grid subroutines.

On tripole\index{grid!tripole} grids, the order of operations used for calculating elements of the stress tensor can differ on either side of the fold, leading to round-off differences.  Although restarts using the extended grid routines are exact for a given run, the solution will differ from another run in which restarts are written at different times.  For this reason, explicit halo updates of the stress tensor are implemented for the tripole grid, both within the dynamics calculation and for restarts.  This has not been implemented yet for tripoleT grids, pending further testing.
...
The restart files supplied with the code release include the core variables on the default configuration, that is, with seven vertical layers and the ice thickness distribution\index{thickness!distribution} defined by {\tt kcatbound=0}.   Restart information for some tracers is also included in the \netcdf\ restart files.
...
An additional namelist option, {\tt restart\_ext} specifies whether halo cells\index{halo} are included in the restart files.  This option is useful for tripole and regional grids, but can not be used with PIO.\index{grid!horizontal}\index{grid!regional}\index{grid!tripole}\index{PIO}
\begin{table}
\begin{center}
...
In the classic EVP or EAP approach ({\tt kdyn} = 1 or 2, {\tt revised\_evp} = true), the dynamics component is subcycled {\tt ndte} {($N$)} times
...
For the revised EVP approach ({\tt kdyn} = 1, {\tt revised\_evp} = false), the relaxation parameter {\tt arlx1i}  effectively sets the damping timescale\index{damping timescale} in the problem, and {\tt brlx} represents the effective subcycling \cite{BFLM:13}.  In practice the parameters $S_e>0.5$ and $\xi<1$ are set, along with an estimate of the ice strength\index{strength} per unit mass, and the damping and subcycling parameters are then calculated.   With the addition of the revised EVP approach to CICE, the code now uses these parameters internally for both classic and revised EVP configurations (see Section~\ref{subsubsec:revp}).
...
see \cite{HZ:99} for its practical effects. The thermodynamics component is stable for any time step, as long as the surface temperature $T_{\mathit sfc}$ is computed internally. \index{temperature!surface}  The numerical constraint on the thermodynamics time step is associated with the transport scheme rather than the thermodynamic solver.
\index{elastic!-viscous-plastic dynamics|)}
\index{stability|)}
\index{subcycling|)}
...
If {\tt write\_ic} is set to true in {\bf ice\_in}, a snapshot of the same set of history fields at the start of the run will be written to the history directory in {\bf iceh\_ic.[timeID].nc(da)}. \index{initial condition}  Several history variables are hard-coded for instantaneous output regardless of the averaging flag,  at the frequency given by their namelist flag. 
...
Like {\tt histfreq}, the parameter {\tt diagfreq} can be used to
...
\index{conservation}%
conservation of mass and energy.  {\tt print\_points} writes data for two specific grid points.  Currently, one point is near the North Pole and the other is in the Weddell Sea; these may be changed in {\bf ice\_in}.
...
\index{restart|(emph}%
CICE now provides restart data in binary unformatted or \netcdf\ formats, via the {\tt IO\_TYPE} flag in {\bf comp\_ice} and namelist variable {\tt restart\_format}.  Restart and history files must use the same format.  As with the history output, there is also an option for writing parallel \netcdf\ restart files using PIO.\index{PIO}

The restart files created by CICE  contain all of the variables 
 needed for a full, exact restart. The filename begins with the
character string `iced.', and the restart dump frequency is
given by the namelist variables {\tt dumpfreq} and {\tt dumpfreq\_n}.  The pointer to the filename from which the
restart data is to be read for a continuation run is set in {\tt pointer\_file}.
The code assumes that auxiliary binary tracer\index{tracers} restart files will be identified using the same pointer and file name prefix, but with an additional character string in the file name that is associated with each tracer set.  All variables are included in \netcdf\ restart files.

Additional namelist\index{namelist} flags provide further control of restart behavior.  {\tt dump\_last} = true causes a set of restart files to be written at the end of a run when it is otherwise not scheduled to occur.  The flag {\tt use\_restart\_time} enables the user to choose to use the model date provided in the restart files.  If {\tt use\_restart\_time} = false then the initial model date stamp is determined from the namelist parameters. {\tt lcdf64} = true sets 64-bit \netcdf\ output, allowing larger file sizes with \netcdf\ version 3.

Routines for gathering, scattering and (unformatted) reading and writing 
of the ``extended" global grid,\index{grid!extended} including the physical domain and ghost (halo)\index{halo} cells around the outer edges, allow exact restarts on regional \index{grid!regional}
grids with open boundary conditions,\index{boundary!condition} and they will also simplify restarts on 
the various tripole grids.\index{grid!tripole}  They are accessed by setting {\tt restart\_ext} = true in namelist.  Extended grid restarts are not available when using PIO; in this case extra halo update calls fill ghost cells for tripole grids (do not use PIO for regional grids).

Two \netcdf\ restart files are included with the CICE v5 code distribution, for the gx3 and gx1 grids.  The were created using the default model configuration (settings as in {\bf comp\_ice} and {\bf ice\_in}), but initialized with no ice.  The gx3 case was run for 1 year using the 1997 forcing data provided with the code.  The gx1 case was run for 20 years, so that the date of restart in the file is 1978-01-01.  Note that the restart dates provided in the restart files can be overridden using the namelist variables {\tt use\_restart\_time}, {\tt year\_init} and {\tt istep0}.  The forcing time can also be overridden using {\tt fyear\_init}.

Several changes in CICE v5 have made restarting from v4.1 restart files difficult.  First, the ice and snow enthalpy state variables are now carried as tracers instead of separate arrays, and salinity has been added as a necessary restart field.   Second, the default number of ice layers has been increased from 4 to 7.  Third, netcdf format is now used for all I/O; it is no longer possible to have history output as \netcdf\ and restart output in binary format.  However, some facilities are included with CICE v5 for converting v4.1 restart files to the new file structure and format, provided that the same number of ice layers and basic physics packages will be used for the new runs.  See Section \ref{subsection:restarttrouble} for details.
\index{restart|)}
...
\begin{enumerate} 
\item Download the forcing data used for testing from the CICE website, 
%\href{http://climate.lanl.gov/Models/CICE/}{http://climate.lanl.gov/Models/CICE/}.
\href{http://oceans11.lanl.gov/trac/CICE/wiki}{http://oceans11.lanl.gov/trac/CICE/wiki}. 
...
A more flexible option is available for double-precision MPI calculations, using the namelist variable {\tt bfbflag}.  When true, this flag produces bit-for-bit identical diagnostics with different tasks, threads, blocks and grid decompositions.\index{reproducible}  
...
that {\tt ndte} is an integer. (This is done differently for {\tt revised\_evp} = true.; see Section~\ref{subsec:dynamics})
...
\index{ice!fraction} changes, but this option usually improves timings even for relatively small processor counts.  T. Craig has found that performance improved by more than 20\% for  combinations of updated decompositions and masked haloes, in CESM's version of CICE.\index{CESM/CCSM}  A practical guide for choosing a CICE grid decomposition, based on experience in CESM, is available:\\
\href{http://oceans11.lanl.gov/drupal/CICE/DecompositionGuide}{http://oceans11.lanl.gov/drupal/CICE/DecompositionGuide}
...
possible, and for backward compatibility of binary restart files, each new tracer has its own binary 
...
\begin{enumerate}
\item {\bf ice\_domain\_size.F90}:  increase {\tt max\_ntrcr} (can also add option to {\bf comp\_ice} and {\bf bld/Macros.*})
\item {\bf ice\_state.F90}:  declare {\tt nt\_[tracer]} and {\tt tr\_[tracer]} 
...
\item {\bf ice\_init.F90}: (some of this may be done in {\bf ice\_[tracer].F90} instead)
\begin{itemize}  
...
\item define tracer types ({\tt trcr\_depend} = 0 for ice area tracers, 1 for ice volume, 2 for snow volume, 2+nt\_[tracer] for dependence on other tracers)
\end{itemize}
...
\begin{itemize}
\item call routine to write tracer restart data
\item call physics routines in {\bf ice\_[tracer].F90} (often called from {\bf ice\_step\_mod.F90})
\end{itemize}
\item {\bf ice\_restart.F90}:  define restart variables\index{restart} (for binary, \netcdf\ and PIO)
...
\label{sec:debug}
Check the FAQ: 
\href{http://oceans11.lanl.gov/drupal/CICE/FAQ}{http://oceans11.lanl.gov/drupal/CICE/FAQ}.
...
\item if starting from a restart file, ensure that kcatbound is the same as that used to create the file ({\tt kcatbound} = 0 for the files included in this code distribution).  Other configuration parameters, such as {\tt NICELYR}, must also be consistent between runs.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Restarts}
\label{subsection:restarttrouble}
...
Binary restart files that were provided with CICE v4.1 were made using the BL99 thermodynamics with 4 layers and 5 thickness categories ({\tt kcatbound} = 0) and therefore can not be used for the default CICE v5 configuration (7 layers).  In addition, CICE's default restart file format is now \netcdf\ instead of binary.

Restarting a run using {\tt runtype} = `continue' requires restart data for all tracers used in the new run.  If tracer restart data is not available, use {\tt runtype} = `initial', setting {\tt ice\_ic} to the name of the core restart file and setting to true the namelist restart flags for each tracer that is available.  The unavailable tracers will be initialized to their default settings.

On tripole grids, use {\tt restart\_ext} = true when using either binary or regular (non-PIO) netcdf.

Provided that the same number of ice layers (default: 4) will be used for the new runs, it is possible to convert v4.1 restart files to the new file structure and then to \netcdf\ format.  If the same physical parameterizations are used, the code should be able to execute from these files.  However if different physics is used (for instance, mushy thermo instead of BL99), the code may still fail.  To convert a v4.1 restart file:
\begin{enumerate}
\item Edit the code {\bf input\_templates/convert\_restarts.f90} for your model configuration and path names.  Compile and run this code to create a binary restart file that can be read using v5.  Copy the resulting file to the {\bf restart/} subdirectory in your working directory.
\item In your working directory, turn off all tracer restart flags in {\bf ice\_in} and set the following:
\begin{itemize}
\item runtype = `initial'
\item ice\_ic = `./restart/[your binary file name]'
\item restart = .true.
\item use\_restart\_time = .true.
\end{itemize}
\item In {\bf CICE\_InitMod.F90}, comment out the call to restartfile(ice\_ic) and uncomment the call to restartfile\_v4(ice\_ic) immediately below it.  This will read the v4.1 binary  file and write a v5 \netcdf\ file containing the same information.
\end{enumerate}  
...
On some architectures, underflows ($10^{-300}$ for example) are not flushed to zero automatically.  Usually a compiler flag is available to do this, but if not, try uncommenting the block of code at the end of subroutine {\it stress} in {\bf ice\_dyn\_evp.F90} or {\bf ice\_dyn\_eap.F90}.  You will take a hit for the extra computations, but it will not be as bad as running with the underflows.

In some configurations, multiple calls to scatter or gather global variables may overfill MPI's buffers, causing the code to slow down (particularly when writing large output files such as restarts).\index{MPI}  To remedy this problem, set {\tt BARRIERS yes} in {\bf comp\_ice}.   This synchronizes MPI messages, keeping the buffers in check.
...
timestepping loop (see {\bf CICE\_RunMod.F90\_debug}, which can be substituted for {\bf CICE\_RunMod.F90}).  
...
\item  With the old CCSM radiative scheme ({\tt shortwave} = `default' or `ccsm3'), a sizable fraction (more than 10\%) of the total 
\index{radiation!shortwave}%
...
\item Latitude and longitude fields in the history output may be wrong when using padding.
\end{enumerate}
...
With the addition of several alternative parameterizations for sea ice processes, a number of subprocesses now appear in multiple parts of the code with differing descriptions.  For instance, sea ice porosity\index{liquid fraction} and  permeability\index{permeability},  along with  associated flushing\index{drainage!flushing} and flooding, are calculated separately for mushy thermodynamics,\index{thermodynamics!mushy} topo  and level-ice melt ponds,\index{melt pond} and for the brine height\index{height!brine} tracer, each employing its own equations.  Likewise, the BL99 and mushy thermodynamics  compute  freeboard\index{freeboard} and snow--ice\index{snow!snow--ice} formation differently, and the topo and level-ice melt pond schemes both allow  fresh ice to grow atop melt ponds, using slightly different formulations for Stefan freezing.\index{Stefan growth}  These various process parameterizations will be compared and their subprocess descriptions possibly unified in the future.
...
\item Andrew Roberts of the Naval Postgraduate School,
...
\item Sylvain Bouillon for the revised EVP approach,
\item the many researchers who tested beta versions of CICE 5 and
...
\hline
&&&\\
{\it setup\_nml} & &{\it Time, Diagnostics} & \\
{\tt days\_per\_year} & {\tt 360} or {\tt 365} & number of days in a model year & {365} \\
...
{\tt ice\_ic} & {\tt default} & latitude and sst dependent & {default} \\
         & {\tt none} & no ice & \\
...
{\tt restart\_format} & nc & read/write \netcdf\ restart files (use with PIO) \\
                                 & bin & read/write binary restart files \\
{\tt lcdf64} & true/false & if true, use 64-bit \netcdf\ format & \\
...
{\tt dumpfreq} & {\tt y} & write restart every {\tt dumpfreq\_n} years & {y}\\
...
&&&\\
&&{\it Model Output}& \\
{\tt bfbflag} & true/false & for bit-for-bit diagnostic output & \\
...
{\tt histfreq} & string array & defines output frequencies & \\
         ...
         & {\tt x}  & unused frequency stream (not written) &\\
...
&&{\it Domain, continued}& \\
 {\tt distribution\_type} & {\tt cartesian} & distribute blocks in 2D Cartesian array & \\      
                                & {\tt roundrobin} & 1 block per proc until blocks are used & \\ 
                                & {\tt sectcart} & blocks distributed to domain quadrants & \\ 
                                & {\tt sectrobin} & several blocks per proc until used & \\ 
 ...
 {\tt ew\_boundary\_type} & {\tt cyclic} & periodic boundary conditions in x-direction & \\
                                      & {\tt open}  & Dirichlet boundary conditions in x & \\
 {\tt ns\_boundary\_type} & {\tt cyclic} & periodic boundary conditions in y-direction & \\
                                      & {\tt open}  & Dirichlet boundary conditions in y & \\
...
&&&\\
{\it thermo\_nml}&&{\it Thermodynamics} \\
 {\tt kitd} & {\tt 0} & delta function ITD approximation & {1}\\
...
\hline
&&&\\
&&{\it Thermodynamics, continued}& \\
{\tt a\_rapid\_mode} & real & brine channel diameter & $0.5\times 10^{-3}$ m  \\
{\tt Rac\_rapid\_mode} & real & critical Rayleigh number & 10 \\
{\tt aspect\_rapid\_mode} & real & brine convection aspect ratio & 1 \\
{\tt dSdt\_slow\_mode} & real & drainage strength parameter & $-1.5\times 10^{-7}$ m/s/K \\
{\tt phi\_c\_slow\_mode} & $0<\phi_c < 1$ & critical liquid fraction & 0.05 \\
{\tt phi\_i\_mushy} & $0<\phi_i < 1$ & solid fraction at lower boundary & 0.85 \\
&&&\\
{\it dynamics\_nml}&&{\it Dynamics}& \\
 {\tt kdyn} & {\tt 0} &  dynamics OFF & {1}\\
     & {\tt 1} & EVP dynamics &\\
     & {\tt 2} & EAP dynamics &\\
...
{\tt advection} & {\tt remap}   & linear remapping advection & {`remap'}\\
          & {\tt upwind}  & donor cell advection &\\
{\tt kstrength} & {\tt 0} & ice strength formulation \cite{Hib:79}  & {1}\\
          & {\tt 1} & ice strength formulation \cite{Rothridge:75} & \\
{\tt krdg\_partic} & {\tt 0} & old ridging participation function & {1} \\
                 & {\tt 1} & new ridging participation function &  \\
{\tt krdg\_redist} & {\tt 0} & old ridging redistribution function & {1} \\
                 & {\tt 1} & new ridging redistribution function &  \\
{\tt mu\_rdg} & real & e-folding scale of ridged ice & \\
{\tt  Cf} & real      & ratio of ridging work to PE change in ridging{\dotfill\ } & 17. \\

&&&\\
{\it shortwave\_nml}&&{\it Shortwave}& \\
{\tt shortwave} & {\tt default} & \index{NCAR} NCAR CCSM3 distribution method & \\
                    & {\tt dEdd} & Delta-Eddington method & \\
{\tt albedo\_type} & {\tt default} & \index{NCAR} NCAR CCSM3 albedos & {`default'} \\
...
{\tt kalg} & real & absorption coefficient for algae & \\
\hline
\end{tabular}
\caption{Namelist options (continued).\index{namelist|emph}}
\end{center}
...
{\it zbgc\_nml}&&{\it Biogeochemistry}& \\
 ...
 {\tt skl\_bgc} & true/false & biogeochemistry & \\
...
& {\tt constant} & constant ice--ocean flux velocity & \\
 ...
 {\tt nit\_data\_type} & {\tt default} & default forcing value for nitrate & \\
 ...
 {\tt phi\_snow} & real  & snow porosity for brine height tracer  & \\
&&&\\
{\it forcing\_nml}&&{\it Forcing}& \\
{\tt formdrag} & true/false & calculate form drag &  \\
{\tt atmbndy} & {\tt default} & stability-based boundary layer & {`default'}\\
...
{\tt highfreq} & true/false & high-frequency atmo coupling & \\
{\tt natmiter} & integer & number of atmo boundary layer iterations & \\
...
{\tt tfrz\_option} & {\tt minus1p8} & constant ocean freezing temperature ($-1.8\degC$) & \\
                            & {\tt linear\_salt} & linear function of salinity (ktherm=1) & \\
                            & {\tt mushy\_layer} & matches mushy-layer thermo (ktherm=2) & \\
{\tt ustar\_min} & real & minimum value of ocean friction velocity & 0.0005 m/s \\
{\tt fbot\_xfer\_type} & {\tt constant} & constant ocean heat transfer coefficient & \\
                                   & {\tt Cdn\_ocn} & variable ocean heat transfer coefficient & \\
%&&{\it Forcing, continued}&\\
{\tt update\_ocn\_f} & true & include frazil water/salt fluxes in ocn fluxes & \\
                          & false & do not include (when coupling with POP) & \\
{\tt l\_mpond\_fresh} & true & retain (topo) pond water until ponds drain  & \\
...
a\_rapid\_mode& {$\bullet$}brine channel diameter &   \\
...
aspect\_rapid\_mode & {$\bullet$} brine convection aspect ratio {\dotfill\ }& 1 \\
...
awtidf   & weighting factor for near-ir, diffuse albedo{\dotfill\ } & 0.36218 \\
 awtidr   & weighting factor for near-ir, direct  albedo{\dotfill\ } & 0.00182 \\
 awtvdf  & weighting factor for visible, diffuse albedo{\dotfill\ } & 0.63282 \\
 awtvdr  & weighting factor for visible, direct  albedo {\dotfill\ }& 0.00318 \\
&&\\
{\Large\bf B} &&\\
&&\\
bfb\_flag &  {$\bullet$} for bit-for-bit reproducible diagnostics & \\
bgc\_data\_dir&  {$\bullet$} data directory for bgc & \\
bgc\_flux\_type &  {$\bullet$} ice--ocean flux velocity  & \\
...
Cdn\_ocn & ocean drag coefficient & \\
 Cf      & {$\bullet$} ratio of ridging work to PE change in ridging{\dotfill\ } & 17. \\
...
\begin{center}
\begin{tabular*}{6.5in}{@{}l@{\extracolsep{\fill}}l@{\extracolsep{\fill}}l@{}} 
 distribution\_type & {$\bullet$} method used to distribute blocks on processors & \\
...
dSdt\_slow\_mode & {$\bullet$} drainage strength parameter &  \\
...
dT\_mlt & {$\bullet$} $\Delta$ temperature  per $\Delta$ snow grain radius{\dotfill\ }  & 1. deg\\
...
dump\_last & {$\bullet$} if true, write restart on last time step of simulation & \\
...
\begin{center}
\begin{tabular*}{6.5in}{@{}l@{\extracolsep{\fill}}l@{\extracolsep{\fill}}l@{}} 
fbot\_xfer\_type & {$\bullet$} type of heat transfer coefficient under ice &\\
...
ffracn & fraction of fsurfn used to melt pond ice & \\
 ...
 fhocn\_ai   & grid-box-mean net heat flux to ocean (fhocn) &W/m$^2$ \\
 ...
 floeshape & floe shape constant for lateral melt {\dotfill\ } & 0.66 \\
...
formdrag & {$\bullet$} calculate form drag &  \\
 ...
 fresh\_ai   & grid-box-mean fresh water flux (fresh) &kg/m$^2$/s \\
frz\_onset & day of year that freezing begins & \\
 frzmlt  & freezing/melting potential &W/m$^2$ \\
 frzmlt\_init & freezing/melting potential at beginning of time step &W/m$^2$ \\
 ...
 fsalt\_ai  & grid-box-mean salt flux to ocean (fsalt) & kg/m$^2$/s \\
...
\begin{tabular*}{6.5in}{@{}l@{\extracolsep{\fill}}l@{\extracolsep{\fill}}l@{}} 
 ...
 fswthru\_ai & grid-box-mean shortwave penetrating to ocean (fswthru) &W/m$^2$\\
...
&&\\
 ...
 grid\_type         & {$\bullet$} `rectangular', `displaced\_pole', `column' or `regional' &\\
 gridcpl\_file         & {$\bullet$} input file for coupling grid info & \\
 grow\_ net & specific biogeochemistry growth rate per grid cell & s$^{-1}$ \\
...
hfrazilmin  & minimum thickness of new frazil ice  {\dotfill\ }& 0.05 m \\
 ...
 highfreq &  {$\bullet$} high-frequency atmo coupling {\dotfill\ } & F \\
 ...
 hp0 & pond depth at which shortwave transition to bare ice occurs {\dotfill\ }& 0.2 m \\
 hp1 & {$\bullet$} critical ice lid thickness for topo ponds (dEdd) {\dotfill\ }& 0.01 m \\
 hpmin & minimum melt pond depth (shortwave) {\dotfill\ }& 0.005 m \\
...
kalg & {$\bullet$} absorption coefficient for algae & \\
 ...
 kice      & thermal conductivity of fresh ice (BL99){\dotfill\ }& 2.03 W/m/deg \\
...
natmiter & {$\bullet$} number of atmo boundary layer iterations {\dotfill\ } & 5 \\
...
nbtrcr & number of biology tracers \index{tracers} & \\
ncat    & number of ice categories {\dotfill\ } & 5\\
...
nit\_data\_type & {$\bullet$}   forcing type for nitrate & \\
...
ntrcr & number of tracers \index{tracers} & \\
...
&&\\
 ...
 p666 & 2/3 & \\
 p75 & 3/4 & \\
phi\_c\_slow\_mode & {$\bullet$} critical liquid fraction & \\
phi\_i\_mushy & {$\bullet$} solid fraction at lower boundary &  \\
phi\_sk & skeletal layer porosity & \\
phi\_snow & {$\bullet$} snow porosity for brine height tracer  & \\
 pi & $\pi$ & \\
 pi2         &  $2\pi$ & \\ 
 pih & $\pi /2$ & \\
 piq & $\pi /4$ & \\
...
pndaspect & {$\bullet$} aspect ratio of pond changes (depth:area)  {\dotfill\ } & 0.8 \\
 ...
 PP\_net & total primary productivity per grid cell & mg C/m$^2$/s \\
...
Rac\_rapid\_mode & {$\bullet$} critical Rayleigh number{\dotfill\ } & 10 \\
...
revp & real(revised\_evp) & \\
  ...
  restart\_bgc & {$\bullet$} if true, read bgc restart file & \\
  restart\_dir &{$\bullet$} path to restart/dump files & \\
  restart\_file &{$\bullet$} restart file prefix & \\
  restart\_format & {$\bullet$}  restart file format &  \\
...
\begin{center}
\begin{tabular*}{6.5in}{@{}l@{\extracolsep{\fill}}l@{\extracolsep{\fill}}l@{}} 
  restore\_bgc & {$\bullet$} if true, restore nitrate/silicate to data & \\
 ...
 rhosi              & average sea ice density (for hbrine tracer) {\dotfill\ }& 940.   kg/m$^3$ \\
 rhow              & density of seawater {\dotfill\ }& 1026.  kg/m$^3$ \\
 rnilyr  &  real(nlyr) &\\
 rside & fraction of ice that melts laterally & \\
 rsnw\_fresh & freshly fallen snow grain radius {\dotfill\ } & 100. $\times$ 10$^{-6}$  m \\
 rsnw\_melt & {$\bullet$} melting snow grain radius {\dotfill\ } & 1000. $\times$ 10$^{-6}$  m \\
...
salinz & ice salinity profile & ppt \\
 saltmax   & max salinity, at ice base (BL99)   {\dotfill\ }  & 3.2 ppt \\
...
sil\_data\_type & {$\bullet$}  forcing type for silicate & \\
...
skl\_bgc & {$\bullet$} biogeochemistry on/off & \\
...
Tf      & freezing temperature & C\\
 ...
 tfrz\_option & {$\bullet$} form of ocean freezing temperature & \\
 ...
 tr\_bgc\_[tracer] & {$\bullet$} if true, use biogeochemistry tracers & \\
 tr\_brine & {$\bullet$} if true, use brine height tracer & \\
\end{tabular*}
\end{center}
\begin{center}
\begin{tabular*}{6.5in}{@{}l@{\extracolsep{\fill}}l@{\extracolsep{\fill}}l@{}} 
 ...
 Tsf\_errmax & max allowed $T_{\mathit sf}$ error (thermodynamics)  {\dotfill\ } & 5.$\times$10$^{-4}$deg\\
...
use\_restart\_time & {$\bullet$} if true, use date from restart file & \\
...
&&\\
 ...
 viscosity\_dyn & dynamic viscosity of brine{\dotfill\ }  & $1.79\times 10^{-3}$ kg/m/s \\
...
zTrf    & reference height for $T_{ref}$, $Q_{ref}$, $U_{ref}$ {\dotfill\ } & 2. m \\
