...
Software User's Manual \\ Version 5.0  \\ LA-CC-06-012 }
...
It updates CICE 4.1, which was released in May 2010: 
\begin{itemize}
...
\item[{\bf   ice\_history\_drag}] history output of form drag variables\index{history}
...
the sea ice model may exchange coupling fluxes using a different grid than the computational grid.  This functionality is activated using the namelist variable {\tt gridcpl\_file}.
\begin{figure}
\centerline{
...
\begin{enumerate}
\item {\bf ice\_domain\_size.F90}:  increase {\tt max\_ntrcr} (can also add option to {\bf comp\_ice})
...
\end{itemize}
\item {\bf ice\_restart.F90}:  define restart variables\index{restart} (for \netcdf\ and PIO)
...
\begin{enumerate}
\item Edit the code {\bf input\_templates/convert\_restarts.f90} for your model configuration and path names.  Compile and run this code to create a binary restart file that can be read using v5.0.  Copy the resulting file to the {\bf restart/} subdirectory in your working directory.
...
\end{itemize}
\item In {\bf CICE\_InitMod.F90}, comment out the call to restartfile(ice\_ic) and uncomment the call to restartfile\_v4(ice\_ic) immediately below it.  This will read the v4.1 binary  file and write a v5.0 \netcdf\ file containing the same information.
\end{enumerate}  
