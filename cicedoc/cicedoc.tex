...
Software User's Manual \\ Version 5.0 {\color{red} DRAFT} \\ LA-CC-06-012 {\color{red}RENEW}} 
...
\begin{itemize}
\item Two methods for prognosing sea ice salinity
...
\item New history variables for melt ponds, ridging diagnostics, biogeochemistry, etc.
...
temperature, $T_f=-\mu S$, where $S$ is the seawater 
\index{salinity!ocean}%
salinity and
$\mu=0.054 \ ^\circ$/ppt is the ratio of the freezing temperature of
brine to its salinity.  The ocean model performs this calculation; if
the 
...
%}  
The dynamics component employs a ``replacement pressure''\index{replacement pressure}
...
The thermodynamic sea ice model is based on \cite{MU:71}
and \cite{BL:99}, and is described more fully in
\cite{Lipscomb:98}. For each thickness category the model computes
...
The vertical 
salinity profile is prescribed and is
unchanging in time.
...
\index{temperature!ice}%
temperature and 
\index{salinity!ice|emph}%
salinity.  Since the salinity
is prescribed, there is a one-to-one relationship between
temperature and enthalpy. We can also define a 
snow enthalpy
$q_s$, which depends on temperature alone. {Expressions for the
enthalpy are derived in Section~\ref{subsubsec:thermo_growth}.}
...
advances these quantities to time~$m+1$.  The calculation proceeds
...
the submerged portion of the floe $h_{sub}$, and hydraulic head $\Delta H$.
...
reduction in solid fraction \cite{EGPRF:04}. The hydraulic head of melt water 
on sea ice (i.e., its height above sea level) drives flushing of melt water through the 
porous sea ice and into the underlying ocean. We model the vertical flushing rate using 
Darcy's law for flow through a porous medium
\begin{equation}
\label{eq:topo_darcy}
w = - \frac{\Pi_v}{\mu} \rho_o g \frac{\Delta H}{h_i},
\end{equation}
where $w$ is the vertical mass flux per unit perpendicular crosssectional area 
(i.e., the vertical component of the Darcy velocity), $\Pi_v$ is the vertical component 
of the permeability tensor (assumed to be isotropic in the horizontal), $\mu$ is the viscosity of water, $\rho_o$ is the ocean 
...
can be calculated from the liquid fraction $\phi$:
\begin{equation}
...
A. The {\tt frzpnd = `hlid'} option uses a Stefan approximation for growth of fresh ice and is invoked only when $\Delta V_{melt}=0$.  
...
A pressure head is computed as $P=g\rho_w\Delta h$ where $\Delta h$ is the height of the pond and sea ice above sea level.    Then the volume of water drained is given by 
...
Given the temperatures $T_{\mathit{sf}}^m$, $T_s^m$, and $T_{ik}^m$ at time~$m$,
...
the latent heat of fusion of fresh ice at \zeroC, and $\mu = 0.054$~deg/ppt is the ratio between the freezing temperature
and 
\index{salinity!ice}%
salinity of brine. 

\index{conductivity|(emph}
...
conductivity is also reduced to bring the layer temperature just to the 
melting point.
...
We first derive expressions for the enthalpy $q$.  The enthalpy of
snow (or fresh ice) is given by
\[ q_s(T) = - \rho_s (-c_0 T + L_0). \]
Sea ice enthalpy is more complex, because of brine
pockets whose 
\index{salinity!ice}%
salinity varies inversely with temperature.
...
If we let $T^\prime = T_{m} \equiv -\mu S$, the temperature
...
\label{eq:top_melting} q \, \delta h = \left\{\begin{array}{ll}
                 ...
                 \right.
\end{equation}
where $q$ is the enthalpy of the surface ice or snow layer (recall
...
In this case we raise the snow base to sea level
...
\begin{description}
\item[{\bf cice4/}]  official driver for CICE v.4 (LANL)
  \begin{description}
  ...
  \item[{\bf   shr\_orb\_mod.F90}]  orbital parameterizations based on ``shared" code in CESM
  \end{description}
\end{description}
...
\item[{\bf   col/}] column configuration files\index{column configuration}
  ...
  \end{description}
  \item[{\color{red}\bf eap\_stresses?}]
\item[{\bf   gx1/}] $\left<1^\circ\right>$ displaced pole grid\index{grid} files
  ...
  \item[{\bf   iced\_gx1\_v5.0}] restart file used for initial condition
  \end{description}
\item[{\bf   gx3/}] $\left<3^\circ\right>$ displaced pole grid\index{grid} files
  ...
  \item[{\bf   ice.restart\_file}] pointer for restart file name\index{restart}
  \item[{\bf   ice\_in}] namelist input data (data paths depend on particular system)
  \item[{\bf   iced\_gx3\_v5.0}] restart file used for initial condition
  \end{description}
...
\item[{\color{red}\bf   ice\_algae.F90}] 
...
\item[{\color{red}\bf   ice\_brine.F90}] 
...
\item[{\bf   ice\_dyn\_eap.F90}] \index{elastic!-anisotropic-plastic dynamics}\index{anisotropic}%
...
\item[{\bf   ice\_restart\_<tracer>.F90}]  read/write tracer restart file\index{restart}\index{tracer}
...
\item[{\color{red}RM?\bf   ice\_spacecurve.F90}]  space-filling-curves distribution method\index{space-filling curve}
...
\item[{\bf   ice\_.F90}] \index{history}
\item[{\bf   ice\_therm\_0layer.F90}] zero-layer thermodynamics of \cite{Semt:76}\index{thermodynamics!zero-layer}
\item[{\bf   ice\_therm\_bl99.F90}] multilayer thermodynamics of \cite{BL:99}\index{thermodynamics!Bitz and Lipscomb 1999}
...
\item[{\color{red}RM\bf   ice\_therm\_oned.F90}] \index{thermodynamics!one-dimensional}
...
\item[{\color{red}\bf ice\_zbgc.F90}] \index{biogeochemistry}
\item[{\color{red}\bf ice\_zbgc\_shared.F90}] \index{biogeochemistry}
\item[{\color{red}\bf ice\_zsalinity.F90}] \index{biogeochemistry}
\end{description}
...
\begin{description}
\item[{\bf iced\_gx3\_v4.0\_kcatbound0}]  initial condition from {\bf cice/input\_templates/gx3/}
...
considerations, large time steps ({\it eg.,} 
$\Delta t=1$~day, 
...
\hspace{-0.5in}
\centering
\includegraphics[scale=0.45, angle=0, viewport=-10 20 350 340,clip]{/Users/eclare/Modeling/CICE/timings/distrb/distrb_cart_X1_20x24_16.eps}
\includegraphics[scale=0.45, angle=0, viewport=30 20 350 340,clip]{/Users/eclare/Modeling/CICE/timings/distrb/distrb_cart_X2_20x24_16.eps}
\includegraphics[scale=0.45, angle=0, viewport=30 20 350 340,clip]{/Users/eclare/Modeling/CICE/timings/distrb/distrb_cart_sqr_20x24_16.eps}\\
\hspace{-0.5in}
\includegraphics[scale=0.45, angle=0, viewport=-10 20 350 360,clip]{/Users/eclare/Modeling/CICE/timings/distrb/distrb_rake_block_20x24_16.eps}
\includegraphics[scale=0.45, angle=0, viewport=30 20 350 360,clip]{/Users/eclare/Modeling/CICE/timings/distrb/distrb_rake_lat_20x24_16.eps}
\includegraphics[scale=0.45, angle=0, viewport=30 20 350 360,clip]{/Users/eclare/Modeling/CICE/timings/distrb/distrb_sfc_lat_20x24_16.eps}
\begin{picture}(0,0)
...
\begin{figure}
\centering
\includegraphics[scale=.9, viewport=0 480 500 720,clip]{/Users/eclare/Modeling/CICE/timings/optimization/timings_20080404/timings.pdf} \\
\includegraphics[scale=.9, viewport=0 0 500 240,clip]{/Users/eclare/Modeling/CICE/timings/optimization/timings_20080404/timings.pdf}
\caption{\label{fig:timings} 
(a) CICE 4.0 timings for  8, 16, 32, and 64 processors of the Linux cluster ``{\color{red}coyote}," after 240  1-hr time steps on a 320x384 displaced pole
grid.  (b) 8-processor timings for various processor shapes (indicated by the upper pair of numbers; eg., 8x1 = `slenderX1') and block sizes.  }
\end{figure}
...
Figure~\ref{fig:timings}  shows the model's scaling
...
\item for 
\index{flux coupler}%
coupled runs, check that {\tt -Dcoupled} and other coupled-model-specific (eg., CESM, popcice or hadgem) preprocessing options are set in the {\bf Macros.*} file.   
...
{\tt diagfreq} & integer & frequency of diagnostic output in {\tt dt} & 24 \\
         & {\it eg.}, 10    & once every 10 time steps &\\
...
&&&\\
{\color{red}\it mushy\_nml}&&&\\
{\color{red}\it zbgc\_nml}&&&\\
...
{\tt Tfrzpt} & {\tt constant} & freezing temperature = -1.8$\degC$ & \\
           & {\tt linear\_S} & linear function of salinity profile & \\
...
f\_$\left<var\right>$ & string & frequency units for writing $\left<var\right>$  to history & \\
         & {\tt y} & write  history every  {\tt histfreq\_n} years & \\
         & {\tt m} & write history every  {\tt histfreq\_n} months &\\
         & {\tt d} & write history every  {\tt histfreq\_n} days &\\
         & {\tt h} & write history every  {\tt histfreq\_n} hours &\\
...
alv(n)dr(f)\_gbm{\color{red}AI}   & grid-box-mean value of alv(n)dr(f) & \\
...
hfrazilmin  & minimum thickness of new frazil ice  {\dotfill\ }& 0.05 m \\
 ...
 hsmin  & minimum snow depth for Delta-Eddington{\dotfill\ } & 1.$\times$10$^{-4}$ m \\
...
Tf      & freezing temperature & C\\
 ...
 Tfrzpt & {$\bullet$} type of freezing temperature (`constant' or `linear\_S') & \\
...
\index{dynamics!elastic-viscous-plastic|see{elastic-viscous-plastic}}
