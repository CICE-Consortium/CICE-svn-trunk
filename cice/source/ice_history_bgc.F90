...
! 2012 Elizabeth Hunke split code from ice_history.F90
      ...
      use ice_domain_size, only: max_nstrm, max_aero, n_aero, nblyr, &
                        n_algae, max_dic, max_doc, max_don, &
                        max_algae, n_dic, n_doc, n_don, n_zaero, &
                        max_fe, n_fed, n_fep  
      use ice_zbgc_shared
...
!=======================================================================
      ...
      use ice_state, only: tr_aero, tr_brine, nbtrcr
     ...
     if (.not. solve_zsal)     f_bgc_S  = 'x'
      ...
      if (f_fzaero(1:1) /= 'x') then
          do n = 1, n_zaero
            ...
            call define_hist_field(n_fzaero(n,:),vname_in,"mmol/m^2/s",tstr2D, tcstr, &
      ...
      if (f_fzaero_ai(1:1) /= 'x') then
          do n = 1, n_zaero
            ...
            call define_hist_field(n_fzaero_ai(n,:),vname_in,"mmol/m^2/s",tstr2D, tcstr, &
      ...
      if (f_zaeronet(1:1) /= 'x')  then
          do n = 1, n_zaero
            ...
            write(vname_in,'(a,a)') 'zaero_net', trim(nchar)
             call define_hist_field(n_zaeronet(n,:),vname_in,"mmol/m^2",tstr2D, tcstr,        &
             "Net z aerosol",                     &
             "weighted by ice area ", c1, c0,       &
             ns, f_zaeronet)
          enddo
      ...
      if (f_zaerosnow(1:1) /= 'x')  then
          do n = 1, n_zaero 
            ...
            write(vname_in,'(a,a)') 'zaero_snow', trim(nchar)
             call define_hist_field(n_zaerosnow(n,:),vname_in,"mmol/m^2",tstr2D, tcstr,        &
             "Snow z aerosol",                     &
             "weighted by ice area ", c1, c0,       &
             ns, f_zaerosnow)
          enddo
...
! write average ice quantities or snapshots
      ...
      use ice_state, only: trcrn, trcr, aicen, aice, vice, vicen, nt_aero, nt_fbri, &
      ...
      if (f_iki   (1:1) /= 'x') then
         ...
         do n = 1, ncat_hist
            do j = jlo, jhi
               do i = ilo, ihi
                if (aicen(i,j,n,iblk) > c0) then
                   workz(i,j,1:nblyr+1) = workz(i,j,1:nblyr+1) + iki(i,j,1:nblyr+1,n,iblk)*&
                       aicen(i,j,n,iblk)
             ...
             do j = jlo, jhi
               do i = ilo, ihi
                  if (aice(i,j,iblk) > puny) then  
                    workz(i,j,1:2) =  &   !snow
                         trcr(i,j,nt_zaero(k)+nblyr+1:nt_zaero(k)+nblyr+2,iblk)
                    workz(i,j,3:nblyr+3) = &  !ice
                         trcr(i,j,nt_zaero(k):nt_zaero(k)+nblyr,iblk)
                    workz(i,j,nblyr+4) =  ocean_bio(i,j,nlt_zaero(k),iblk) !ocean
                  endif
                enddo ! i
             enddo    ! j 
...
!----------------------------------------------------------------------------
       ...
       if (f_zaero(1:1) /= 'x') then
         do n=1,n_zaero
            ...
            call define_hist_field(n_zaeros(n,:),vname_in,"mmol/m^3",tstr3Da, tcstr, &
                "bulk z aerosol conc. ", "snow+bio grid", c1, c0, &
                ns, f_zaero)
          enddo
       endif
