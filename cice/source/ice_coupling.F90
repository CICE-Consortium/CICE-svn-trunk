...
! #endif at end of module
   ...
   integer (kind=int_kind), dimension (cpl_fields_ibuf_total) :: &
      isbuf,            &! coupler send    control buffer
      irbuf              ! coupler receive control buffer

   real (kind=dbl_kind), dimension(:,:), allocatable :: &
      sbuf               ! send buffer

   real (kind=dbl_kind) :: &! send and receive buffers for fields
      buffs(nx_block*ny_block*max_blocks,cpl_fields_i2c_total), &
      buffr(nx_block*ny_block*max_blocks,cpl_fields_c2i_total)

   type(cpl_contract) :: &
      contractS,         &! coupler send contract
      contractR           ! coupler receive contract
...
!
     ...
     &     '(ice_coupling,init_cpl) send initial msg. set contract'
      ...
      isbuf(cpl_fields_ibuf_lsize  ) = nx_block*ny_block ! size of local grid
      isbuf(cpl_fields_ibuf_lisize ) = nx_block  ! local size wrt i-index
      isbuf(cpl_fields_ibuf_ljsize ) = ny_block  ! local size wrt i-index
      ...
      allocate(sbuf(nx_block*ny_block*max_blocks,
     &              cpl_fields_grid_total) )
      ...
      do iblk = 1, nblocks
         ...
         do i = 1, nx_block
            ...
            sbuf(n,cpl_fields_grid_area ) = tarea(i,j,iblk)/(radius**2)
            sbuf(n,cpl_fields_grid_mask ) = float(nint(hm(i,j,iblk)))
...
!
     ...
     &     (cpl_fields_cplname, contractR, irbuf, buffr)
      ...
      do iblk = 1, nblocks
       ...
       do i = 1, nx_block
         ...
         !--- ocn states--
         sst  (i,j,iblk) = buffr(n,cpl_fields_c2i_ot)
         sss  (i,j,iblk) = buffr(n,cpl_fields_c2i_os)
         uocn (i,j,iblk) = buffr(n,cpl_fields_c2i_ou)
         vocn (i,j,iblk) = buffr(n,cpl_fields_c2i_ov)

         !--- atm states-
         zlvl (i,j,iblk) = buffr(n,cpl_fields_c2i_z)
         uatm (i,j,iblk) = buffr(n,cpl_fields_c2i_u)
         vatm (i,j,iblk) = buffr(n,cpl_fields_c2i_v)
         potT (i,j,iblk) = buffr(n,cpl_fields_c2i_ptem)
         Tair (i,j,iblk) = buffr(n,cpl_fields_c2i_tbot)
         Qa   (i,j,iblk) = buffr(n,cpl_fields_c2i_shum)
         rhoa (i,j,iblk) = buffr(n,cpl_fields_c2i_dens)

         !--- ocn states--
         ss_tltx(i,j,iblk) = buffr(n,cpl_fields_c2i_dhdx)
         ss_tlty(i,j,iblk) = buffr(n,cpl_fields_c2i_dhdy)
         frzmlt (i,j,iblk) = buffr(n,cpl_fields_c2i_q)

         !--- atm fluxes--
         swvdr(i,j,iblk) = buffr(n,cpl_fields_c2i_swvdr)
         swidr(i,j,iblk) = buffr(n,cpl_fields_c2i_swndr)
         swvdf(i,j,iblk) = buffr(n,cpl_fields_c2i_swvdf)
         swidf(i,j,iblk) = buffr(n,cpl_fields_c2i_swndf)
         flw  (i,j,iblk) = buffr(n,cpl_fields_c2i_lwdn)
         frain(i,j,iblk) = buffr(n,cpl_fields_c2i_rain)
         fsnow(i,j,iblk) = buffr(n,cpl_fields_c2i_snow)
 ...
 100  format ('comm_diag',1x,a3,1x,a4,1x,i3,es26.19)
     ...
     &                     - workx*sin(ANGLET(i,j,iblk))
         ...
         sxest(i,j,iblk) = sst(i,j,iblk) - Tffresh    ! sea sfc temp (C)
         Tf (i,j,iblk) = -1.8_dbl_kind                ! hardwired for NCOM
c        Tf (i,j,iblk) = -depressT*sss(i,j,iblk)      ! freezing temp (C)
c        Tf (i,j,iblk) = -depressT*max(sss(i,j,iblk),ice_ref_salinity)
...
!EOP
     ...
     &                   + workx*sin(ANGLET(i,j,iblk))
      ...
      isbuf(cpl_fields_ibuf_lisize)  = nx_block
      isbuf(cpl_fields_ibuf_ljsize)  = ny_block
      isbuf(cpl_fields_ibuf_ncpl)    = nadv_i  ! number of msg-pairs per day
      isbuf(cpl_fields_ibuf_lsize)   = nx_block*ny_block*maxblocks
      isbuf(cpl_fields_ibuf_dead)    = 0       ! not a dead model
      ...
      if (flag) then
         do iblk = 1, nblocks
            ...
            do i = 1, nx_block
               if (tmask(i,j,iblk) .and. ailohi(i,j,iblk) < c0 ) then
                  write(nu_diag,*)
     &              '(ice) send: ERROR ailohi < 0.',i,j,ailohi(i,j,iblk)
                  call shr_sys_flush(nu_diag)
               endif
            end do
            end do
      ...
      do iblk = 1, nblocks
       ...
       do i = 1, nx_block
         ...
         !--- ice states
         buffs(n,cpl_fields_i2c_ifrac) = ailohi(i,j,iblk) ! frac

         !---  compression index
         buffs(n,cpl_fields_i2c_index) = rndex_global(i,j) ! global index
         if (tmask(i,j) .and. ailohi(i,j) > c0 ) then
            buffs(n,cpl_fields_i2c_t    ) = Tsrf(i,j,iblk) ! temperature

            buffs(n,cpl_fields_i2c_avsdr) = alvdr(i,j,iblk) ! alb, visible/dir
            buffs(n,cpl_fields_i2c_anidr) = alidr(i,j,iblk) ! alb, near-ir/dir
            buffs(n,cpl_fields_i2c_avsdf) = alvdf(i,j,iblk) ! alb, visible/dif
            buffs(n,cpl_fields_i2c_anidf) = alidf(i,j,iblk) ! alb, near-ir/dif

           !--- a/i fluxes computed by ice
            buffs(n,cpl_fields_i2c_taux ) = tauxa(i,j,iblk) ! stress: a/i zonal
            buffs(n,cpl_fields_i2c_tauy ) = tauya(i,j,iblk) ! stress: a/i merid
            buffs(n,cpl_fields_i2c_lat  ) = flat (i,j,iblk) ! latent heat flux
            buffs(n,cpl_fields_i2c_sen  ) = fsens(i,j,iblk) ! sensible heat flux
            buffs(n,cpl_fields_i2c_lwup ) = flwout(i,j,iblk)! upward longwave flux
            buffs(n,cpl_fields_i2c_evap ) = evap (i,j,iblk) ! evaporation h2o flux
            buffs(n,cpl_fields_i2c_tref ) = Tref (i,j,iblk) ! diagnostic: 2m ref temp
            buffs(n,cpl_fields_i2c_qref ) = Qref (i,j,iblk) ! diagnostic: 2m ref sp hum
            buffs(n,cpl_fields_i2c_swnet) = fswabs(i,j,iblk)! sw net absorbed hf

           !--- i/o fluxes computed by ice
            buffs(n,cpl_fields_i2c_swpen) = fswthru(i,j,iblk) ! solar thru ice to ocn hf
            buffs(n,cpl_fields_i2c_melth) = fhocn(i,j,iblk) ! hf from melting
            buffs(n,cpl_fields_i2c_meltw) = fresh(i,j,iblk) ! h2o flux from melting
            buffs(n,cpl_fields_i2c_salt ) = fsalt(i,j,iblk) ! salt flux from melting
            buffs(n,cpl_fields_i2c_otaux) = tauxo(i,j,iblk) ! stress : i/o zonal
            buffs(n,cpl_fields_i2c_otauy) = tauyo(i,j,iblk) ! stress : i/o meridional
         endif  ! tmask and ailohi > c0
       enddo                    ! i
       enddo                    ! j
      enddo                     ! iblk
...
#endif
! ifdef coupled (previous five subroutines)
!=======================================================================
